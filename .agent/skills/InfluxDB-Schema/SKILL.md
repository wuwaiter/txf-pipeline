...
InfluxDB 是時序資料庫，資料以 **Measurement + Tags + Fields + Timestamp** 組成一筆 Point。本專案透過 `src/app/services/influx_client.py` 的 `get_write_api()` / `get_query_api()` 取得連線。

> 本專案的 Bucket 設計、Measurement Schema 與 Timeframe 對照表，定義於 `txf-architecture/SKILL.md` 的「InfluxDB 資料層規範」章節。
