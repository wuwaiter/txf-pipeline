...
def perform_login(api):
    if not SHIOAJI_API_KEY:
        print(">>> ERROR: SHIOJI_API_KEY not found in .env")
        return False
...
def run_ingest():
    ...
    def on_quote_fop(exchange, quote):
        ...
        r.xadd(stream_key, {"price": str(quote.close), "ts": str(int(time.time()))})
    ...
    def check_and_update_status(write_influx: bool = False):
        ...
        try:
            ...
            # 只有定時檢查才寫入 InfluxDB monitoring bucket
            if write_influx:
                try:
                    point = (
                        Point("shioaji_usage")
    ...
    check_and_update_status(write_influx=False)  # 啟動時只更新畫面，不寫 InfluxDB
